...
#normal distriubtion and plotting
x = rnorm(100)
plot(x)
hist(x)
...
#Read in and Explore a Public Dataset

```{r, echo=TRUE, fig.width=7, fig.height=7, tidy=TRUE, tidy.opts=5}
...
plot(log(dat1$Control_rep1, 10), log(dat1$Control_rep2,10), xlab = "Control Replicate 1", ylab = "Control Replicate2", main = "Comparison of Replicates on Log Scale base 10", pch=19, col=rgb(red=0.0, green=0.5, blue=1.0, alpha=0.2), cex = 0.25)
...
library(reshape2)
FPKM_melt <- melt(log(FPKM_Matrix_Filt_Norm[-c(1:2)]))

library(ggplot2)
ggplot(FPKM_melt, aes(x=variable, y=value)) +
...
#Perform Calculations need for Volcano Plot

```{r, echo=TRUE, fig.width=7, fig.height=7, tidy=TRUE, tidy.opts=5}
## Calculations needed for Volcano Plot
...
## number of genes by each value determination
dim(dat1.3)
dim(dat2.3)
dim(dat3.3)
